...
APP.i18n = (function () {
  var STRINGS = {
    en: {
      ...
      analyzer_disclaimer: 'The part-of-speech tags and pronunciations here are generated by rule, not looked up in a dictionary. Common words are exact; anything else is a best guess from spelling patterns and may miss real exceptions.',
      ...
      analyzer_translate_privacy: 'Google Translate loads automatically as you type (via its public web endpoint, no key needed — unofficial, so it can be slow, rate-limited, or occasionally unavailable). DeepL needs your own API key, entered below and stored only in this browser. Either way, your typed sentence is sent to that service to be translated.',
    ...
    fr: {
      ...
      analyzer_disclaimer: 'Les étiquettes grammaticales et les prononciations ici sont générées par des règles, pas recherchées dans un dictionnaire. Les mots courants sont exacts ; le reste est une estimation à partir des modèles orthographiques et peut manquer de vraies exceptions.',
      analyzer_placeholder: 'Écrivez une phrase en français…',
      ...
      analyzer_translate_privacy: 'Google Traduction se charge automatiquement pendant que vous tapez (via son point d’accès web public, sans clé nécessaire — non officiel, donc parfois lent, limité ou temporairement indisponible). DeepL nécessite votre propre clé API, saisie ci-dessous et stockée uniquement dans ce navigateur. Dans les deux cas, votre phrase tapée est envoyée à ce service pour être traduite.',
