...
APP.i18n = (function () {
  var STRINGS = {
    en: {
      ...
      analyzer_disclaimer: 'Part-of-speech tags and pronunciations come from a curated dictionary first, then from the fr-compromise and espeak-ng libraries once they finish loading, and from spelling-pattern rules as a fallback while they load. All three can still miss real exceptions.',
      ...
      analyzer_translate_privacy: 'Google Translate loads automatically as you type, via its public web endpoint. It is unofficial, so it can be slow, rate-limited, or occasionally unavailable, and it needs no key. DeepL needs your own API key, entered below and stored only in this browser. Either way, your typed sentence is sent to that service to be translated.',
      ...
      analyzer_deepl_key_note: 'Stored only in this browser (localStorage), never in the site’s code or sent anywhere but DeepL’s API. Get a free key at deepl.com/pro-api.',
    ...
    fr: {
      ...
      analyzer_disclaimer: 'Les étiquettes grammaticales et les prononciations viennent d’abord d’un dictionnaire sélectionné, puis des bibliothèques fr-compromise et espeak-ng une fois chargées, et de règles orthographiques par défaut pendant le chargement. Les trois peuvent manquer de vraies exceptions.',
      analyzer_placeholder: 'Écrivez une phrase en français…',
      ...
      analyzer_translate_privacy: 'Google Traduction se charge automatiquement pendant que vous tapez, via son point d’accès web public. Il est non officiel, donc parfois lent, limité ou temporairement indisponible, et ne nécessite aucune clé. DeepL nécessite votre propre clé API, saisie ci-dessous et stockée uniquement dans ce navigateur. Dans les deux cas, votre phrase tapée est envoyée à ce service pour être traduite.',
      ...
      analyzer_deepl_key_note: 'Stockée uniquement dans ce navigateur (localStorage), jamais dans le code du site ni envoyée ailleurs qu’à l’API de DeepL. Obtenez une clé gratuite sur deepl.com/pro-api.',
